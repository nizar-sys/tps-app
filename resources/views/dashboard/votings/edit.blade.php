@extends('layouts.app')
@section('title', 'Ubah Data TPS')

@section('title-header', 'Ubah Data TPS')
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('home') }}">Dashboard</a></li>
    <li class="breadcrumb-item"><a href="{{ route('votings.index') }}">Data TPS</a></li>
    <li class="breadcrumb-item active">Ubah Data TPS</li>
@endsection
        ...
        <div class="col-12">
            <div class="card shadow">
                <div class="card-header bg-transparent border-0 text-dark">
                    <h5 class="mb-0">Formulir Ubah Data TPS</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('votings.update', $voting->id) }}" method="POST" role="form" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                                    ...
                                    <select class="form-control @error('village_id') is-invalid @enderror" id="village_id"
                                        ...
                                        @foreach ($villages as $village)
                                            <option value="{{ $village->id }}"
                                                @if (old('village_id', $voting->village_id) == $village->id) selected @endif>
                            ...
                            <div class="col-sm-12 col-md-12">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control @error('nama_tps') is-invalid @enderror"
                                        id="nama_tps" placeholder="Nama TPS" value="{{ old('nama_tps', $voting->nama_tps) }}"
                                        name="nama_tps">
                                    ...
                                    <select class="form-control @error('tim_id') is-invalid @enderror" id="tim_id"
                                        ...
                                        @foreach ($oneTimVoting as $oneTime)
                                            <option value="{{ $oneTime->id }}"
                                                @if (old('tim_id', $voting->tim_id) == $oneTime->id) selected @endif>
                                                {{ $oneTime->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('tim_id')
