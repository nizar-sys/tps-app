...
@section('title-header', 'Ubah Data Kecamatan')
@section('breadcrumb')
        ...
        <div class="col-12">
            <div class="card shadow">
                <div class="card-header bg-transparent border-0 text-dark">
                    <h5 class="mb-0">Formulir Ubah Data Kecamatan</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('districts.update', $district->id) }}" method="POST" role="form" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                                    ...
                                    <select class="form-control @error('region_id') is-invalid @enderror" id="region_id"
                                        ...
                                        @foreach ($regions as $region)
                                            <option value="{{ $region->id }}"
                                                @if (old('region_id', $district->region_id) == $region->id) selected @endif>
                            ...
                            <div class="col-sm-12 col-md-12">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control @error('nama_kecamatan') is-invalid @enderror"
                                        id="nama_kecamatan" placeholder="Nama Kecamatan" value="{{ old('nama_kecamatan', $district->nama_kecamatan) }}"
                                        name="nama_kecamatan">
                                    ...
                                    <select class="form-control @error('mentor_id') is-invalid @enderror" id="mentor_id"
                                        ...
                                        @foreach ($mentors as $mentor)
                                            <option value="{{ $mentor->id }}"
                                                @if (old('mentor_id', $district->mentor_id) == $mentor->id) selected @endif>
                                                {{ $mentor->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('mentor_id')
